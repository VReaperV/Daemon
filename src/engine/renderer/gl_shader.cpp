...
std::string GLShaderManager::ShaderPostProcess( GLShader *shader, const std::string& shaderText ) {
	...
	std::string texDataBlock = "struct TexData {\n"
	                           ...
	                           "	uvec2 u_DeluxeMap;\n"
	                           "};\n\n"
	                           "layout(std140, binding = 7) uniform lightmapDataSSBO {\n"
	...
	*/
	...
	for( GLUniform* uniform : shader->_uniforms ) {
		...
		/* if ( uniform->UseTexData() ) {
			materialDefines += " texData[baseInstance >> 16].";
		} else {
			materialDefines += " materials[baseInstance & 0xFFFF].";
		} */
