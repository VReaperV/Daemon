...
class GLUniform {
	...
	uint32_t* GetPushBufferData( const uint32_t size ) {
		if ( _updateType <= FRAME ) {
			return pushBuffer.GetGlobalUniformData() + pushOffset;
		}

		pushBuffer.pushStart = std::min( pushBuffer.pushStart, pushOffset );
		pushBuffer.pushEnd = std::max( pushBuffer.pushEnd, pushOffset + size );
...
public:
	...
	void RegisterUniform( GLUniform* uniform );
	static GLuint SortUniforms( std::vector<GLUniform*>& uniforms, const bool setOffset, const uint32_t bufferOffset );
	static std::vector<GLUniform*> ProcessUniforms( const GLUniform::UpdateType minType, const GLUniform::UpdateType maxType,
		...
		const bool setOffset = false, const uint32_t offset = 0 );
...
class GLUniformSampler : protected GLUniform {
	...
	void SetValueBindless( GLint64 value ) {
		...
		if ( glConfig2.usingBindlessTextures ) {
			...
			// memcpy( GetPushBufferData( _std430Size ), &value, sizeof( GLuint64 ) );
			// return;
		}
...
protected:
	...
	inline void SetValue( int value )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			*( int* ) GetPushBufferData( _std430Size ) = value;
			return;
		}
...
class GLUniform1ui : protected GLUniform {
	...
	GLUniform1ui( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniform( shader, name, "uint", 1, 1, updateType, 0, false ) {
	}
	...
	inline void SetValue( uint value ) {
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			*GetPushBufferData( _std430Size  ) = value;
			return;
		}
...
protected:
	...
	inline void SetValue( float value )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			*( float* ) GetPushBufferData( _std430Size ) = value;
			return;
		}
...
protected:
	...
	inline void SetValue( int numFloats, float *f )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			memcpy( GetPushBufferData( numFloats * _std430Size ), f, numFloats * sizeof( float ) );
			return;
		}
...
protected:
	...
	inline void SetValue( const vec2_t v )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			Vector2Copy( v, *( vec2_t* ) GetPushBufferData( _std430Size ) );
			return;
		}
...
protected:
	...
	inline void SetValue( const vec3_t v )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			VectorCopy( v, *( vec3_t* ) GetPushBufferData( _std430Size ) );
			return;
		}
...
protected:
	...
	inline void SetValue( const vec4_t v )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			Vector4Copy( v, *( vec4_t* ) GetPushBufferData( _std430Size ) );
			return;
		}
...
protected:
	...
	inline void SetValue( int numV, vec4_t *v )
	{
		...
		if ( _shader->UseMaterialSystem() && _updateType == MATERIAL_OR_PUSH || glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			memcpy( currentValue.data(), v, numV * sizeof( vec4_t ) );
			return;
		}

		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			memcpy( GetPushBufferData( numV * _std430Size ), v, numV * sizeof( vec4_t ) );
			return;
		}
...
protected:
	...
	inline void SetValue( GLboolean transpose, const matrix_t m )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			MatrixCopy( m, ( vec_t* ) GetPushBufferData( _std430Size ) );
			return;
		}
...
class GLUniformMatrix32f : protected GLUniform {
	...
	inline void SetValue( GLboolean transpose, const vec_t* m ) {
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			uint32_t* data = GetPushBufferData( _std430Size );
...
protected:
	...
	inline void SetValue( int numMatrices, GLboolean transpose, const matrix_t *m )
	{
		...
		if ( glConfig2.pushBufferAvailable && _updateType <= FRAME ) {
			memcpy( GetPushBufferData( numMatrices * _std430Size ), m, numMatrices * sizeof( matrix_t ) );
			return;
		}
...
class u_PushBufferArea :
	GLUniform1ui {
	public:
	u_PushBufferArea( GLShader* shader ) :
		GLUniform1ui( shader, "u_PushBufferArea", SKIP ) {
	}
