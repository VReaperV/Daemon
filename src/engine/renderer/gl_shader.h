...
private:
	...
	uint32_t pushUniformSize = 0;
	uint32_t pushUniformPadding = 0;
...
public:
	...
	void WriteGlobalUniformsToBuffer( uint32_t* buffer );
};
...
extern std::unordered_multimap<std::string, GLUniform*> allUniforms;
extern std::vector<GLUniform*> globalUniforms;
class GLUniform {
	public:
	enum UpdateType {
	...
	static GLuint _pushSize;
	static GLuint _pushAlignment;
	...
	GLUniform( GLShader* shader, const char* name, const char* type, const GLuint std430Size, const GLuint std430Alignment,
		...
		const bool isTexture = false, bool newPass = false ) :
		_shader( shader ),
		_name( name ),
		_type( type ),
		...
		if ( glConfig2.globalUBOAvailable ) {
			...
			if ( std::find_if( globalUniforms.begin(), globalUniforms.end(),
				...
				globalUniforms.push_back( this );
			}

			allUniforms.insert( { _name, this } );
		}
	}
	...
	uint32_t* GetPushUniformData( const uint32_t size ) {
		pushUBO.pushStart = std::min( pushUBO.pushStart, pushOffset );
		pushUBO.pushEnd = std::max( pushUBO.pushEnd, pushOffset + size );

		return pushUBO.GetPushUniformData() + pushOffset;
	}
};
...
public:
	...
	void RegisterUniform( GLUniform* uniform );
	static GLuint SortUniforms( std::vector<GLUniform*>& uniforms, const bool setOffset );
	static std::vector<GLUniform*> ProcessUniforms( const GLUniform::UpdateType minType, const GLUniform::UpdateType maxType,
		const bool skipTextures, std::vector<GLUniform*>& uniforms, GLuint& structSize, GLuint& padding, const bool setOffset = false );
...
class GLUniformSampler : protected GLUniform {
	protected:
	GLUniformSampler( GLShader* shader, const char* name, const char* type, const GLuint size, const UpdateType updateType ) :
		GLUniform( shader, name, type, glConfig2.bindlessTexturesAvailable ? size * 2 : size,
		                               glConfig2.bindlessTexturesAvailable ? size * 2 : size, updateType, 0, true ) {
	}

	inline GLint GetLocation() {
		ShaderProgramDescriptor* p = _shader->GetProgram();

		if ( _updateType >= PUSH || !_shader->UseMaterialSystem() ) {
	...
	void SetValueBindless( GLint64 value ) {
		...
		if ( glConfig2.usingBindlessTextures && glConfig2.globalUBOAvailable ) {
			if ( _shader->UseMaterialSystem() && _updateType == TEXDATA_OR_PUSH ) {
				return;
			}

			// memcpy( GetPushUniformData( _std430Size ), &value, sizeof( GLuint64 ) );
			// return;
		}
...
class GLUniformSampler2D : protected GLUniformSampler {
	...
	GLUniformSampler2D( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniformSampler( shader, name, "sampler2D", 1, updateType ) {
	}
...
class GLUniformSampler3D : protected GLUniformSampler {
	...
	GLUniformSampler3D( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniformSampler( shader, name, "sampler3D", 1, updateType ) {
	}
...
class GLUniformUSampler3D : protected GLUniformSampler {
	...
	GLUniformUSampler3D( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniformSampler( shader, name, "usampler3D", 1, updateType ) {
	}
...
class GLUniformSamplerCube : protected GLUniformSampler {
	...
	GLUniformSamplerCube( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniformSampler( shader, name, "samplerCube", 1, updateType ) {
	}
...
protected:
	...
	inline void SetValue( int value )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			*( int* ) GetPushUniformData( _std430Size ) = value;
			return;
		}
...
class GLUniform1ui : protected GLUniform {
	protected:
	GLUniform1ui( GLShader* shader, const char* name, const UpdateType updateType, bool pass = true ) :
		GLUniform( shader, name, "uint", 1, 1, updateType, 0, false, pass ) {
	}
	...
	inline void SetValue( uint value ) {
		...
		if ( glConfig2.globalUBOAvailable && pass && _updateType != SKIP ) {
			*GetPushUniformData( _std430Size  ) = value;
			return;
		}
...
protected:
	...
	inline void SetValue( float value )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			*( float* ) GetPushUniformData( _std430Size ) = value;
			return;
		}
...
protected:
	...
	inline void SetValue( int numFloats, float *f )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			memcpy( GetPushUniformData( numFloats * _std430Size ), f, numFloats * sizeof( float ) );
			return;
		}
...
protected:
	...
	inline void SetValue( const vec2_t v )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			Vector2Copy( v, *( vec2_t* ) GetPushUniformData( _std430Size ) );
			return;
		}
...
protected:
	...
	inline void SetValue( const vec3_t v )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			VectorCopy( v, *( vec3_t* ) GetPushUniformData( _std430Size ) );
			return;
		}
...
public:
	...
	uint32_t* WriteToBuffer( uint32_t* buffer ) override {
		...
		return buffer + 3; // vec3 is aligned to 4 components
	}
...
protected:
	...
	inline void SetValue( const vec4_t v )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			Vector4Copy( v, *( vec4_t* ) GetPushUniformData( _std430Size ) );
			return;
		}
...
protected:
	...
	inline void SetValue( int numV, vec4_t *v )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			memcpy( GetPushUniformData( numV * _std430Size ), v, numV * sizeof( vec4_t ) );
			return;
		}
...
protected:
	...
	inline void SetValue( GLboolean transpose, const matrix_t m )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			MatrixCopy( m, ( vec_t* ) GetPushUniformData( _std430Size ) );
			return;
		}
...
class GLUniformMatrix32f : protected GLUniform {
	...
	inline void SetValue( GLboolean transpose, const vec_t* m ) {
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			uint32_t* data = GetPushUniformData( _std430Size );
			memcpy( data, m, 3 * sizeof( float ) );
			memcpy( data + 4, m + 3, 3 * sizeof( float ) );
			return;
		}
...
protected:
	...
	inline void SetValue( int numMatrices, GLboolean transpose, const matrix_t *m )
	{
		...
		if ( glConfig2.globalUBOAvailable && pass ) {
			memcpy( GetPushUniformData( numMatrices * _std430Size ), m, numMatrices * sizeof( matrix_t ) );
			return;
		}
...
class u_ColorMapCube :
	...
	u_ColorMapCube( GLShader* shader ) :
		GLUniformSamplerCube( shader, "u_ColorMapCube", MATERIAL_OR_PUSH ) {
	}
...
class u_LightMap :
	...
	u_LightMap( GLShader* shader ) :
		GLUniformSampler( shader, "u_LightMap", "sampler2D", 1, TEXDATA_OR_PUSH ) {
	}
...
class u_DeluxeMap :
	...
	u_DeluxeMap( GLShader* shader ) :
		GLUniformSampler( shader, "u_DeluxeMap", "sampler2D", 1, TEXDATA_OR_PUSH ) {
	}
...
class u_FogMap :
	...
	u_FogMap( GLShader* shader ) :
		GLUniformSampler2D( shader, "u_FogMap", CONST ) {
	}
...
public:
	u_FogDensity( GLShader *shader ) :
		GLUniform1f( shader, "u_FogDensity", MATERIAL_OR_PUSH )
	{
	}
...
class u_CloudHeight :
	...
	u_CloudHeight( GLShader* shader ) :
		GLUniform1f( shader, "u_CloudHeight", CONST ) {
	}
...
class u_PushUBOArea :
	GLUniform1ui {
	public:
	u_PushUBOArea( GLShader* shader ) :
		GLUniform1ui( shader, "u_PushUBOArea", SKIP ) {
	}

	void SetUniform_PushUBOArea( uint32_t value ) {
		this->SetValue( value );
	}
};
...
class u_ViewWidth :
	...
	u_ViewWidth( GLShader* shader ) :
		GLUniform1ui( shader, "u_ViewWidth", CONST ) {
	}
...
class u_ViewHeight :
	...
	u_ViewHeight( GLShader* shader ) :
		GLUniform1ui( shader, "u_ViewHeight", CONST ) {
	}
...
public:
	u_Time( GLShader *shader ) :
		GLUniform1f( shader, "u_Time", PUSH ) // Source this from a buffer when entity support is added to the material system
	{
	}
...
public:
	u_ColorModulate( GLShader *shader ) :
		GLUniform4f( shader, "u_ColorModulate", MATERIAL_OR_PUSH )
	{
	}
...
class u_ColorModulateColorGen_Uint :
	...
	u_ColorModulateColorGen_Uint( GLShader* shader ) :
		GLUniform1ui( shader, "u_ColorModulateColorGen", MATERIAL_OR_PUSH, false ) {
	}
...
public:
	u_FogDistanceVector( GLShader *shader ) :
		GLUniform4f( shader, "u_FogDistanceVector", PUSH )
	{
	}
...
public:
	u_LightGridOrigin( GLShader *shader ) :
		GLUniform3f( shader, "u_LightGridOrigin", FRAME )
	{
	}
...
public:
	u_LightGridScale( GLShader *shader ) :
		GLUniform3f( shader, "u_LightGridScale", FRAME )
	{
	}
