...
class GLUniformSampler : protected GLUniform {
	...
	void SetValueBindless( GLint64 value ) {
		...
		if ( glConfig2.usingBindlessTextures && !_shader->UseMaterialSystem() && !glConfig2.globalUBOAvailable ) {
...
class GLUniformSampler2D : protected GLUniformSampler {
	...
	inline GLint GetLocation() {
		...
		if ( !glConfig2.globalUBOAvailable && !_shader->UseMaterialSystem() ) {
			ASSERT_EQ( p, glState.currentProgram );
		}
...
protected:
	...
	inline void SetValue( int value )
	{
		...
		if ( _shader->UseMaterialSystem() || glConfig2.globalUBOAvailable ) {
			currentValue = value;
			return;
		}
...
class GLUniformMatrix32f : protected GLUniform {
	...
	GLUniformMatrix32f( GLShader* shader, const char* name, const UpdateType updateType ) :
		GLUniform( shader, name, "mat3x2", 6, 2, updateType ) {
	}
...
class u_LightMap :
	...
	u_LightMap( GLShader* shader ) :
		GLUniformSampler( shader, "u_LightMap", "sampler2D", 1, CONST ) {
	}
...
class u_DeluxeMap :
	...
	u_DeluxeMap( GLShader* shader ) :
		GLUniformSampler( shader, "u_DeluxeMap", "sampler2D", 1, CONST ) {
	}
...
public:
	u_Color_Float( GLShader *shader ) :
		GLUniform4f( shader, "u_Color", MATERIAL_OR_PUSH )
	{
	}
...
public:
	u_ColorGlobal_Float( GLShader *shader ) :
		GLUniform4f( shader, "u_ColorGlobal", PUSH )
	{
	}
...
public:
	u_ColorModulateColorGen_Float( GLShader* shader ) :
		GLUniform4f( shader, "u_ColorModulateColorGen", PUSH )
	{
	}
